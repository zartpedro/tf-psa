    ...
    <div class="max-w-4xl mx-auto bg-white p-8 rounded shadow">
        ...
        <div v-if="reimbursement">
            ...
            <div class="mb-4">
                ...
                <p class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm">{{ reimbursement.date }}
                </p>
            </div>
            <div class="mb-4">
...
const approveReimbursement = async () => {
    await approveReimbursement(reimbursement.value.id);
    router.push('/manager/approve-reimbursements');
};

const rejectReimbursement = async () => {
    if (rejectionReason.value.trim()) {
        await rejectReimbursement(reimbursement.value.id, rejectionReason.value);
